...
const achievements: Achievement[] = [
  ...
  },
  {
    ...
    deskripsi: "Kuasai seluruh alfabet isyarat SIBI.",
    icon: IconLetterCase,
    status: "diraih",
    tanggal: "18 Jun 2025",
    xp: 80,
  ...
  },
  {
    ...
    deskripsi: "Buka semua level di jalur pembelajaran SIBI.",
    icon: IconRocket,
    status: "terkunci",
    syarat: "Buka semua 10 level di jalur SIBI.",
    xp: 150,
    warna: "gray",
  },
]
...
export default function PencapaianPage() {
  ...
  return (
    <div className="px-8 py-8 space-y-6 max-w-7xl w-full mx-auto select-none">
...
function AchievementCardDiraih({ achievement: a, delayClass }: { achievement: Achievement, delayClass: string }) {
  return (
    <div className={`bg-white rounded-2xl border border-gray-100 p-5 flex flex-col gap-4 transition-all duration-300 hover:border-blue-200 hover:shadow-md hover:animate-float-card ${delayClass}`}>
  ...
  return (
    <div className={`bg-white rounded-2xl border border-gray-100 p-5 flex flex-col gap-4 transition-all duration-300 hover:border-amber-200 hover:shadow-md hover:animate-float-card ${delayClass}`}>
      ...
      <div className="bg-blue-50 rounded-xl px-4 py-3 flex items-start gap-2">
        <IconChevronRight size={14} className="text-blue-500 shrink-0 mt-0.5" />
        <p className="text-xs text-blue-700">{a.langkahSelanjutnya}</p>
      </div>
    </div>
  )
}
