...
const levels: Level[] = [
  ...
  { id: 1, title: "Salam & Perkenalan", status: "selesai", materi: 6, selesai: 6, achievement: "Pemberi Salam", xp: 50, deskripsi: "Pelajari cara menyapa, memperkenalkan diri, dan berpamitan menggunakan bahasa isyarat SIBI." },
      ...
      <div className="fixed bottom-6 right-8 flex items-center gap-3 z-40">
        ...
        {halaman === 1 && (
          <button
            ...
            className="flex items-center gap-1 bg-blue-600 hover:bg-blue-700 text-white font-extrabold text-sm px-5 py-3 rounded-xl transition active:scale-95 shadow-lg"
          >
            Level 11 - 20
...
function LevelNode({
  ...
  const styles: Record<LevelStatus, { bg: string; border: string; icon: string; Icon: React.ElementType }> = {
    selesai:      { bg: "bg-blue-50", border: "border-blue-500", icon: "text-blue-600", Icon: IconCheck },
    berjalan:     { bg: "bg-blue-50",    border: "border-blue-500",    icon: "text-blue-600",    Icon: IconPlayerPlay },
  ...
  const warnaGaris = level.id > 10 ? "bg-indigo-100" : "bg-blue-100"
    ...
    <div key={level.id} className="flex flex-col h-full animate-fade-up">
      <div className="flex-1 overflow-y-auto pb-4 pr-1">
        <div className="flex items-center justify-between mb-6">
          <span className="text-sm font-bold text-blue-600 bg-blue-50 px-3 py-1 rounded-full">Level {level.id}</span>
        ...
        <div className="mb-6">
          <div className="flex justify-between text-xs font-semibold text-gray-600 mb-2">
            ...
            <span className="text-blue-600">{level.selesai} dari {level.materi} soal</span>
          </div>
        ...
        <div className="bg-gray-50 border border-gray-100 rounded-2xl px-5 py-4 mb-6">
          ...
          <p className="text-sm font-bold text-gray-800 flex items-center gap-2">
            <IconCircleCheck size={18} className="text-blue-500" />
      ...
      <div className="pt-5 border-t border-gray-100 shrink-0 mt-auto">
        <button
          onClick={() => router.push(`/SIBI/quiz/${level.id}`)}
          className="w-full bg-blue-600 hover:bg-blue-700 hover:shadow-lg hover:-translate-y-0.5 text-white font-bold text-sm rounded-xl py-3.5 transition-all duration-200 flex items-center justify-center gap-2"
        >
          {buttonLabel}
