...
export default function KamusCard({ item, onPlay, liked, onToggleLike }: KamusCardProps) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 p-5 hover:shadow-md transition hover:scale-[1.02] duration-200 flex flex-col gap-3 relative">
      {item.mastered && (
        <div className="select-none absolute top-3 left-3 flex items-center gap-1 bg-blue-50 border border-blue-200 text-blue-600 text-[10px] font-bold px-2 py-0.5 rounded-full justify-center">
      ...
      <div className="flex items-center justify-between mt-auto pt-1">
        ...
        <button
          onClick={() => onPlay(item)}
          className="select-none w-8 h-8 rounded-full border-2 border-gray-200 flex items-center justify-center text-gray-500 hover:border-blue-500 hover:text-blue-800 hover:bg-blue-50 transition text-xs"
        >
          ▶
        </button>
      </div>
