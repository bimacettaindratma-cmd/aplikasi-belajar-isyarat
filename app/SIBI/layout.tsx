      ...
      <aside className="w-64 gap-6 h-screen bg-white border-r border-gray-100 flex flex-col shrink-0">
        <div className="flex flex-col gap-4 px-6 py-6">
          <div className="flex gap-2 items-center">
            <img src="/IsyaratKu biru.png" className="w-10 h-10" alt="Logo" />
            <h1 className="text-xl font-extrabold text-blue-800">IsyaratKu</h1>
          </div>
          <div className="flex bg-blue-100 rounded-2xl border border-[#0284C7] p-2 w-30 items-center justify-center">
            <p className="text-[#0284C7] text-xs font-bold">SIBI Path</p>
          </div>
        </div>

        {/* Menu Navigasi */}
        <nav className="flex-1 px-4 space-y-1.5">
          <SidebarItem icon={IconHome} label="Beranda" href="/SIBI/beranda" />
          <SidebarItem icon={IconMap} label="Materi" href="/SIBI/materi" />
          <SidebarItem icon={IconBook} label="Kamus" href="/SIBI/kamus" />
          <SidebarItem icon={IconLetterCase} label="Alfabet & Angka" href="/SIBI/alfabet" />
          <SidebarItem icon={IconTrophy} label="Quiz" href="/SIBI/quiz" />
          <SidebarItem icon={IconAward} label="Pencapaian" href="/SIBI/pencapaian" />
        </nav>

        {/* Menu Profil di Bagian Bawah */}
        <div className="px-4 pb-6">
          <SidebarItem icon={IconUser} label="Profil" href="/SIBI/profil" />
        </div>
      </aside>
      ...
      <div className="h-screen flex-1 flex flex-col min-w-0">
        ...
        <header className="h-20 bg-white border-b border-gray-100 flex items-center justify-end gap-4 px-8 shrink-0">
          <div className="flex items-center gap-3 shrink-0">
            <span className="flex items-center gap-1.5 bg-blue-50 text-[#0284C7] text-sm font-semibold px-3 py-1.5 rounded-full">
              <IconFlame size={16} className="fill-[#0284C7]" /> 7 Days
            </span>
            <button className="relative w-9 h-9 rounded-full hover:bg-gray-50 flex items-center justify-center text-gray-500">
              <IconBell size={20} />
            </button>
            <Link href="/SIBI/profil" className="inline-block transition hover:opacity-80">
              <img 
