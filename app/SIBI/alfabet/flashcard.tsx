...
export default function FlashCard({ items }: Props) {
  ...
  return (
    <div className="select-none flex flex-col items-center gap-6">
      {/* Card */}
      <div className="w-full max-w-sm border-2 border-blue-300 rounded-3xl bg-white shadow-lg shadow-blue-100 flex flex-col items-center py-10 px-8 gap-4">
        <span className="text-8xl">{current.emoji}</span>
        <p className="text-5xl font-extrabold text-blue-600">{current.label}</p>
      ...
      <div className="flex items-center gap-4">
        ...
        <button
          ...
          className="select-none px-5 py-2.5 rounded-xl bg-blue-600 text-white text-sm font-semibold hover:bg-blue-700 disabled:opacity-30 disabled:cursor-not-allowed transition"
        >
          Berikutnya →
        </button>
      </div>
