...
export default function AlfabetDetail({ item, isClosing, onClose }: Props) {
  return (
    ...
    >
      ...
      >
        <div className="flex justify-center py-6 text-7xl bg-teal-50 rounded-2xl mb-5">
          {item.emoji}
        </div>

        <p className="text-3xl font-extrabold text-teal-600 text-center mb-1">{item.label}</p>
        ...
        <button
          onClick={onClose}
          className="select-none w-full py-3 rounded-xl bg-teal-600 text-white font-semibold text-sm hover:bg-teal-700 transition"
        >
          ✓ Selesai
        </button>
      </div>
