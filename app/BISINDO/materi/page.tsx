    ...
    <div className="p-6 md:p-8 max-w-7xl mx-auto space-y-10 select-none">
      ...
      <div className="animate-fade-up">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden flex flex-col md:flex-row">
          ...
          {/* Sisi Kiri */}
          <div className="relative w-full md:w-[42%] min-h-[220px] bg-slate-800 flex items-center justify-center shrink-0">
            <div className="w-14 h-14 rounded-full bg-teal-600 flex items-center justify-center text-white shadow-lg shadow-teal-900/20">
              <IconPlayerPlay size={24} className="fill-white ml-0.5" />
            </div>
            <span className="absolute bottom-4 left-4 bg-teal-500 text-white text-xs font-bold px-3 py-1.5 rounded-xl shadow-sm">
              Sedang Berlangsung
            </span>
          </div>
          ...
          <div className="flex-1 p-6 md:p-8 flex flex-col justify-between">
            <div>
              <p className="text-teal-600 text-xs font-extrabold tracking-wider uppercase">
            ...
            <div className="mt-6 pt-4 border-t border-gray-50 flex flex-col sm:flex-row sm:items-end justify-between gap-4">
              <div className="flex-1 max-w-md">
                <div className="flex items-center justify-between text-xs font-semibold text-gray-700 mb-1.5">
                  ...
                  <span className="text-teal-600">{continueLearning.progress}%</span>
                </div>
              ...
              <button
                onClick={() => router.push(`/BISINDO/materi/${continueLearning.moduleId}`)}
                className="flex items-center justify-center gap-2 bg-teal-600 text-white font-bold px-6 py-3 rounded-xl text-sm hover:bg-teal-700 transition shadow-sm whitespace-nowrap"
              >
                Lanjutkan Materi
      ...
      <div>
        <div className="flex items-center justify-between mb-2 animate-fade-up">
          ...
          <button className="text-sm text-teal-600 font-semibold hover:underline flex items-center gap-1">
            Lihat Semua
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mt-5">
          {modules.map((modul, index) => {
            ...
            return (
              <div
                key={modul.id}
                className={`bg-white rounded-2xl border border-gray-100 overflow-hidden shadow-sm hover:border-teal-200 transition-all duration-300 flex flex-col justify-between cursor-pointer group hover:animate-float-card ${animationDelayClass}`}
                onClick={() => router.push(`/BISINDO/materi/${modul.id}`)}
              >
                ...
                <div className="p-4 flex-1 flex flex-col justify-between">
                  <div>
                    <h3 className="font-extrabold text-gray-900 text-base mb-1 group-hover:text-teal-600 transition">
                      {modul.title}
                    </h3>
