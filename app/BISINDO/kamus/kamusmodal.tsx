  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Emoji box */}
        <div className="select-none mx-5 mt-3 flex justify-center py-7 text-7xl bg-teal-50 rounded-2xl">
          {item.emoji}
        </div>
        ...
        <div className="px-5 pt-4 pb-5">
          ...
          {/* Steps */}
          <p className="select-none text-[11px] font-bold text-teal-600 uppercase tracking-wide mb-3">Langkah-langkah</p>
          <ol className="space-y-2 mb-5">
            {item.steps.map((step, i) => (
              <li key={i} className="flex items-start gap-3">
                <span className="select-none w-5 h-5 rounded-full bg-teal-700 text-white text-[10px] font-bold flex items-center justify-center shrink-0 mt-0.5">
                  {i + 1}
                </span>
          ...
          {/* CTA */}
          <Link 
            href={`/BISINDO/kamus/${item.id}`} 
            className="block text-center select-none w-full py-3 rounded-xl bg-teal-600 text-white font-bold text-sm hover:bg-teal-700 shadow-md shadow-teal-100 transition"
          >
            🎬 Tonton Video
          </Link>
        </div>
