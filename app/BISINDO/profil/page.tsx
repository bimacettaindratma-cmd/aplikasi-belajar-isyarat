...
export default function ProfilePage() {
  ...
  // Pengaturan POV BISINDO: State default diubah ke BISINDO
  const [activeSystem, setActiveSystem] = useState<"SIBI" | "BISINDO">("BISINDO");
  ...
  // Pengaturan POV BISINDO: Sekarang pop-up digunakan untuk konfirmasi pindah ke SIBI
  const [showSibiModal, setShowSibiModal] = useState(false);
  const [isSibiClosing, setIsSibiClosing] = useState(false);
  ...
  // Pengaturan POV BISINDO: Teks bio diubah fokusnya ke BISINDO
  const [bio, setBio] = useState("Semangat belajar bahasa isyarat! 🤟 Fokus ke BISINDO bulan ini.");

  useEffect(() => {
    return () => {
      // Membersihkan timer jika diperlukan
    };
  }, []);
  ...
  // Pengaturan POV BISINDO: Trigger modal jika user mengklik "SIBI"
  const handleSystemClick = (system: "SIBI" | "BISINDO") => {
    if (system === "SIBI" && activeSystem !== "SIBI") {
      setIsSibiClosing(false);
  ...
  // Pengaturan POV BISINDO: Aksi jika user setuju pindah ke SIBI
  const handleConfirmSibi = () => {
    setIsSibiClosing(true);
    const timer = setTimeout(() => {
      setShowSibiModal(false);
      // Diarahkan ke rute beranda SIBI temanmu
      router.push("/SIBI/beranda");
    }, 300);
    return () => clearTimeout(timer);
  };

  const handleCloseSibiModal = () => {
    setIsSibiClosing(true);
    const timer = setTimeout(() => {
      setShowSibiModal(false);
    }, 300);
  ...
  return (
    <>
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-10 py-10 min-h-screen flex flex-col gap-6 select-none">
          ...
          <div className="w-full bg-white rounded-3xl p-6 flex flex-col sm:flex-row items-center sm:items-start justify-between gap-6 shadow-sm hover:shadow-md transition-shadow duration-200 animate-fade-up">
            <div className="flex flex-col sm:flex-row items-center sm:items-start gap-5 w-full">
              <div className="relative shrink-0">
                <div className="w-20 h-20 rounded-full bg-teal-600 overflow-hidden ring-4 ring-gray-100">
                  <img
                    src="/profile1.jpg"
                    alt="Avatar"
            ...
            <button 
              ...
              className="shrink-0 bg-teal-600 text-white text-xs font-bold px-5 py-2.5 rounded-full hover:bg-teal-700 transition-all active:scale-95 shadow-sm"
            >
              Edit Profil
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 animate-fade-up-delay-1">
            <div className="bg-white rounded-2xl p-5 flex items-center gap-4 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-200">
              <div className="w-12 h-12 rounded-xl bg-teal-50/70 flex items-center justify-center text-xl shrink-0">📘</div>
              <div>
          ...
          <div className="w-full bg-white rounded-3xl p-6 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 animate-fade-up-delay-2">
            ...
            <div className="w-full bg-gray-100 h-3 rounded-full overflow-hidden mb-1.5">
              <div className="bg-teal-600 h-full rounded-full transition-all duration-500 w-[35%]" />
            </div>
            <p className="text-[11px] font-bold text-gray-400">35% Selesai</p>
          </div>

          {/* BARIS 4: Panel Pengaturan */}
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm hover:shadow-md transition-shadow duration-200">
              ...
              <div className="flex items-center justify-between mb-5">
                ...
                <button
                  ...
                  className={`w-11 h-6 rounded-full relative transition-colors duration-200 shrink-0 ${reminder ? "bg-teal-600" : "bg-gray-300"}`}
                >
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm hover:shadow-md transition-shadow duration-200">
              ...
              <div className="flex items-center justify-between mb-5">
                ...
                <div className="flex gap-1 bg-gray-100 p-1 rounded-xl shrink-0">
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-lg text-sm transition-all ${!darkMode ? "bg-white text-teal-600 shadow-sm font-bold" : "text-gray-400 hover:text-gray-600"}`}
                  >
                    ☀️ Terang
                  </button>
                  <button
              ...
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className={`w-11 h-6 rounded-full relative transition-colors duration-200 shrink-0 ${autoLoop ? "bg-teal-600" : "bg-gray-300"}`}
                >
      ...
      {showEditModal && (
        <div
          className={`fixed top-20 left-64 right-0 bottom-0 z-50 flex items-center justify-center px-4 bg-gray-900/20 ${isClosing ? "animate-backdrop-out" : "animate-backdrop-in"}`}
          onClick={handleCloseEditModal}
        >
          <div
            className={`select-none bg-white rounded-3xl shadow-xl w-full max-w-sm p-6 relative ${isClosing ? "animate-modal-out" : "animate-modal-in"}`}
          ...
          >
            <div className="flex items-start justify-between mb-5">
              ...
              <button onClick={handleCloseEditModal} className="w-7 h-7 rounded-full bg-gray-50 flex items-center justify-center text-gray-400 text-sm hover:bg-gray-100 transition active:scale-95">✕</button>
            </div>

            <div className="flex flex-col items-center gap-2 mb-6">
              <div className="relative group cursor-pointer">
                <div className="w-20 h-20 rounded-full bg-teal-600 overflow-hidden ring-4 ring-gray-100 shadow-inner">
                  <img
                    src="/profile1.jpg"
                    alt="Avatar"
                    className="w-full h-full object-cover"
                ...
                <div className="absolute bottom-0 right-0 w-7 h-7 bg-teal-600 rounded-full border-2 border-white flex items-center justify-center text-white text-xs shadow-md">📷</div>
              </div>
              <button className="text-xs font-bold text-teal-600 hover:underline mt-1">Ubah Foto</button>
            </div>

            <div className="space-y-4">
              <div>
                <label className="block text-xs font-bold text-gray-500 mb-1.5 uppercase tracking-wider">Nama Lengkap</label>
                <input type="text" value="Teman Dengar" disabled className="w-full bg-gray-50 border border-gray-100 rounded-xl px-4 py-2.5 text-sm text-gray-400 cursor-not-allowed focus:outline-none" />
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-gray-500 mb-1.5 uppercase tracking-wider">Username</label>
                  <input type="text" value={username} onChange={(e) => setUsername(e.target.value)} className="w-full bg-white border border-gray-200 rounded-xl px-4 py-2.5 text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-600" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-gray-500 mb-1.5 uppercase tracking-wider">Email</label>
                  <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full bg-white border border-gray-200 rounded-xl px-4 py-2.5 text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-600" />
                </div>
              </div>

              <div>
                <label className="block text-xs font-bold text-gray-500 mb-1.5 uppercase tracking-wider">Bio</label>
                <textarea value={bio} onChange={(e) => setBio(e.target.value)} rows={3} maxLength={160} className="w-full bg-white border border-gray-200 rounded-xl px-4 py-2.5 text-sm text-gray-700 resize-none focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-600" />
            ...
            <div className="flex justify-end gap-3 pt-5 mt-5 border-t border-gray-100">
              <button onClick={handleCloseEditModal} className="px-5 py-2 rounded-xl border border-gray-200 text-xs font-bold text-gray-500 hover:bg-gray-50 transition active:scale-95">Batal</button>
              <button onClick={handleCloseEditModal} className="px-5 py-2 rounded-xl bg-teal-600 text-white text-xs font-bold hover:bg-teal-500 transition active:scale-95 flex items-center gap-1.5 shadow-sm">Simpan Perubahan <span className="text-xs">✓</span></button>
            </div>
          </div>
        </div>
      )}

      {/* Pop-up Modal Konfirmasi Pengalihan Rute ke SIBI */}
      {showSibiModal && (
        <div
          className={`fixed top-20 left-64 right-0 bottom-0 z-50 flex items-center justify-center px-4 bg-gray-900/20 ${isSibiClosing ? "animate-backdrop-out" : "animate-backdrop-in"}`}
          onClick={handleCloseSibiModal}
        >
          <div
            className={`select-none bg-white rounded-3xl shadow-xl w-full max-w-sm p-6 relative ${isSibiClosing ? "animate-modal-out" : "animate-modal-in"}`}
            onClick={(e) => e.stopPropagation()}
          >
            <div className="text-center py-4">
              <div className="w-12 h-12 rounded-full bg-teal-50 text-teal-600 flex items-center justify-center text-xl mx-auto mb-4 font-bold">❓</div>
              <h3 className="text-base font-extrabold text-gray-800 px-2">
                Mau pindah belajar bahasa SIBI?
              </h3>
              <p className="text-xs text-gray-400 mt-2 px-4 leading-relaxed">
                Fokus belajar Anda akan dialihkan secara langsung ke modul halaman utama sistem isyarat SIBI.
              </p>
            </div>
            ...
            <div className="flex gap-3 pt-4 mt-2 border-t border-gray-50">
              ...
              <button 
                onClick={handleConfirmSibi}
                className="flex-1 py-2.5 rounded-xl bg-teal-600 text-white text-xs font-bold hover:bg-teal-700 transition active:scale-95 shadow-sm"
              >
                Ya
              </button>
            </div>
