    ...
    <div className="flex flex-col min-h-full bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-100 px-8 py-3 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-2 text-sm text-gray-500">
          <button onClick={() => router.push("/BISINDO/quiz")} className="hover:text-teal-700 transition">
            Quiz
          </button>
        ...
        <button
          onClick={() => router.push("/BISINDO/quiz")}
            ...
            <div className="w-full max-w-3xl grid grid-cols-2 gap-6">
              ...
              <div className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm">
                ...
                <div className="flex flex-col gap-2.5">
                  <button
                    onClick={() => router.push("/BISINDO/quiz")}
                    className="w-full bg-teal-700 hover:bg-teal-800 text-white font-semibold text-sm rounded-xl py-3 transition"
                  >
              ...
              <div className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm flex flex-col items-center justify-center text-center">
                <div className={`w-20 h-20 rounded-full flex items-center justify-center mb-5 ${
                  isPassed ? "bg-teal-600" : "bg-gray-300"
                }`}>
            ...
            <div className="px-8 pt-6 pb-4">
              <div className="max-w-2xl mx-auto w-full">
                ...
                <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
                  <div className="h-full bg-teal-700 rounded-full transition-all duration-300" style={{ width: `${progressPct}%` }} />
                </div>
              </div>
            </div>
            ...
            <div className="flex-1 px-8 pb-6 max-w-2xl mx-auto w-full">
                ...
                {currentQuestion.type === "video" ? (
                  <>
                    ...
                    <div className="absolute bottom-0 left-0 right-0 px-4 py-3 flex items-center gap-3 bg-linear-to-t from-black/60 to-transparent">
                      <button onClick={handleReplay} className="text-white hover:text-teal-300 transition" aria-label="Putar ulang">
                        <IconRotateClockwise size={20} strokeWidth={2} />
                      </button>
                      <div className="flex-1 h-0.5 bg-white/30 rounded-full overflow-hidden">
                        <div className="h-full w-1/2 bg-teal-400 rounded-full" />
                      </div>
                      <button className="text-white hover:text-teal-300 transition" aria-label="Volume">
              ...
              <div className="grid grid-cols-2 gap-3 mb-5">
                {currentQuestion.options.map((option, index) => {
                  ...
                  if (!isAnswered) {
                    cls += isSelected ? "border-teal-500 bg-teal-50 text-teal-800" : "border-gray-200 bg-white text-gray-700 hover:border-gray-300 hover:bg-gray-50";
                  } else {
                    if (isThisCorrect) cls += "border-teal-500 bg-teal-50 text-teal-800";
                  ...
                  return (
                    <button key={index} onClick={() => handleSelectAnswer(index)} disabled={isAnswered} className={cls}>
                      <span className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-semibold shrink-0 ${
                        !isAnswered && isSelected ? "bg-teal-100 text-teal-700"
                        : isAnswered && isThisCorrect ? "bg-teal-100 text-teal-700"
                        : "bg-gray-100 text-gray-500"
                      }`}>
                        {OPTION_LABELS[index]}
                      </span>
                      <span className="flex-1">{option}</span>
                      {isAnswered && isThisCorrect && <IconCheck size={18} className="text-teal-600 shrink-0" strokeWidth={2.5} />}
              ...
              {isCorrect && (
                <div className="flex items-start gap-3 p-4 bg-teal-50 border border-teal-200 rounded-2xl mb-4 text-sm text-teal-800">
                  <IconCheck size={18} className="text-teal-600 mt-0.5 shrink-0" strokeWidth={2.5} />
                  <div>
                    <p className="font-semibold mb-0.5">Jawaban Tepat! ✅</p>
                    <p className="text-teal-700 leading-relaxed">{currentQuestion.explanation}</p>
                  </div>
                </div>
              )}
              {isWrong && (
            ...
            <div className="sticky bottom-0 bg-white border-t border-gray-100 px-8 py-4 mt-auto">
              <div className="flex items-center justify-between max-w-2xl mx-auto">
                ...
                <div className="flex items-center gap-3">
                  {!isAnswered && (
                    <button onClick={handleSkip} className="text-sm font-semibold text-teal-600 hover:text-teal-700 transition px-2">
                      Lewati
                    </button>
                  )}
                  {!isAnswered ? (
                    <button
                      ...
                      className={`flex items-center gap-2 px-6 py-3 rounded-xl text-sm font-semibold transition ${selectedAnswer === null ? "bg-gray-100 text-gray-300 cursor-not-allowed" : "bg-teal-700 hover:bg-teal-800 text-white"}`}
                    >
                      Periksa Jawaban
                    ...
                    <button
                      onClick={goNext}
                      className="flex items-center gap-2 px-6 py-3 rounded-xl text-sm font-semibold bg-teal-700 hover:bg-teal-800 text-white transition"
                    >
